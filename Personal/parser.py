...
def parse_iteration_or_expression(tokens):
    # Parse the left-hand expression first
    left, tokens2 = parse_assignment_expression(tokens)

    # If no 'to', this is a normal expression
    if tokens2[0]["tag"] != "to":
        return left, tokens2

    # Otherwise, this is an iteration call
    tokens2 = tokens2[1:]  # skip 'to'

    # Parse end expression
    end_expr, tokens2 = parse_assignment_expression(tokens2)

    # Default growth rule
    growth_type = "linear"
    growth_expr = None

    # Optional growth spec
    if tokens2[0]["tag"] == "linear":
        growth_type = "linear"
        tokens2 = tokens2[1:]

    elif tokens2[0]["tag"] == "exponential":
        growth_type = "exponential"
        tokens2 = tokens2[1:]
        growth_expr, tokens2 = parse_assignment_expression(tokens2)

    elif tokens2[0]["tag"] == "by":
        growth_type = "custom"
        tokens2 = tokens2[1:]
        growth_expr, tokens2 = parse_assignment_expression(tokens2)

    # Optional iteration variable
    iter_name = "loop"
    if tokens2[0]["tag"] == "using":
        tokens2 = tokens2[1:]
        assert tokens2[0]["tag"] == "identifier", "Expected identifier after 'using'"
        iter_name = tokens2[0]["value"]
        tokens2 = tokens2[1:]

    # Now parse the function call
    call_ast, tokens2 = parse_complex_expression(tokens2)
    assert call_ast["tag"] == "call", "Iteration must end with a function call"
...
def parse_expression(tokens):
    return parse_iteration_or_expression(tokens)
...
def parse_statement_list(tokens):
    ...
    while True:
        ...
        if statement["tag"] == "assign" and statement["value"]["tag"] == "function":
            continue
        if statement["tag"] == "iteration_call":
            continue
...
if __name__ == "__main__":
    ...
    test_functions = old_test_functions

...
def evaluate_iteration_call(ast, environment):
    # Evaluate start and end
    start_val, s_status = evaluate(ast["start"], environment)
    if s_status == "exit":
        return start_val, "exit"

    end_val, e_status = evaluate(ast["end"], environment)
    if e_status == "exit":
        return end_val, "exit"

    current = start_val
    iter_name = ast["iter_name"]
    growth_type = ast["growth_type"]
    growth_expr = ast["growth_expr"]
    ...
    while current <= end_val:
        ...
        loop_env = {"$parent": environment}
        loop_env[iter_name] = current

        # Evaluate the function call
        last_value, call_status = evaluate(ast["call"], loop_env)
        if call_status in ["exit", "return", "break", "continue"]:
            # break/continue inside iteration-call is illegal
            if call_status in ["break", "continue"]:
                raise Exception(f"'{call_status}' not allowed inside iteration-call")
            return last_value, call_status
        ...
        if growth_type == "linear":
            ...
            factor, f_status = evaluate(growth_expr, environment)
            if f_status == "exit":
                return factor, "exit"
            current *= factor

        elif growth_type == "custom":
            # custom growth evaluated in loop scope
            next_val, n_status = evaluate(growth_expr, loop_env)
            if n_status == "exit":
                return next_val, "exit"
            current = next_val

        else:
            raise Exception(f"Unknown growth type '{growth_type}'")
...
def evaluate(ast, environment):
    ...
    if ast["tag"] == "iteration_call":
        return evaluate_iteration_call(ast, environment)
    ...
    if ast["tag"] == "assign":
        ...
        if target["tag"] == "identifier":
            ...
            if target.get("extern"):
                ...
                # Non-extern: always assign in the current environment
...
def equals(code, environment, expected_result, expected_environment=None):
    ...
    if expected_environment != None:
        ...
        clean_expected = clean(expected_environment)

        for key, expected_val in clean_expected.items():
            assert key in clean_env, f"Missing key {key} in environment"
            assert clean_env[key] == expected_val, f"Mismatch for key {key}: got {clean_env[key]}, expected {expected_val}"
...
def test_iteration_linear_with_call():
    print("test_iteration_linear_with_call")
    code = """
        x = 0;
        function addToX(v) { extern x = x + v };
        1 to 5 using i addToX(i);
    """
    equals(code, {"x": 0}, None, {"x": 15})


def test_iteration_linear_implicit_loop():
    print("test_iteration_linear_implicit_loop")
    code = """
        x = 0;
        function addToX(v) { extern x = x + v };
        1 to 3 addToX(loop);
    """
    equals(code, {"x": 0}, None, {"x": 6})  # 1+2+3

def test_iteration_exponential():
    print("test_iteration_exponential")
    code = """
        x = 0;
        function addToX(v) { extern x = x + v };
        1 to 100 exponential 2 addToX(loop);
    """
    # 1 + 2 + 4 + 8 + 16 + 32 + 64 = 127
    equals(code, {"x": 0}, None, {"x": 127})

def test_iteration_custom_by():
    print("test_iteration_custom_by")
    code = """
        x = 0;
        function addToX(v) { extern x = x + v };
        1 to 10 by (loop + 2) addToX(loop);
    """
    # loop: 1, 3, 5, 7, 9  → sum = 25
    equals(code, {"x": 0}, None, {"x": 25})

def test_iteration_propagates_exit():
    print("test_iteration_propagates_exit")
    code = """
        function f(v) { exit 7 };
        1 to 5 f(loop);
    """
    result, status = evaluate(parse(tokenize(code)), {})
    assert result == 7 and status == "exit"
...
if __name__ == "__main__":
    ...
    test_iteration_linear_with_call()
    test_iteration_linear_implicit_loop()
    test_iteration_exponential()
    test_iteration_custom_by()
    test_iteration_propagates_exit()
    print("done.")
